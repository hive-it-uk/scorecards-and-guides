...
- **4.0 (2026)** — as above. *Amended 14 August 2026: the composite's default in section 4's results hierarchy changed from "off by default and shown on request" to "shown by default, always dismissible" — a readability decision; the hierarchy itself and the never-publish-alone rule are unchanged.* *Amended 9 September 2026: criterion 2's water citations closed. AWARE gained its primary source in section 8 — the WULCA method page and Boulay et al. (2018) — and the glossary entry now names the body that developed it. The off-site water proportion is restated from "roughly two-thirds" of the total footprint to about three-quarters of the **operational** footprint for US data centres in 2018, sourced to Siddik, Shehabi and Marston (2021), with a scarcity-weighted counterpoint alongside it: the uncited figure was both wrong and wrong in the direction that understated the problem. No score, criterion, weight or aggregation rule changed.*
...
>
> **The segmented carbon model.** The calculator multiplied the entire 0.30 kWh/GB by the host's grid intensity. Under SWD v4 the host owns only the data-centre segment: 0.067 kWh/GB, 22% of the total. The remaining 78% is network and device energy, consumed where the visitor is, and now uses an audience-weighted intensity set by UK and EU share sliders with the remainder at the world average. Page weight, not hosting choice, is therefore the dominant lever. All three grid figures were brought into line with one Ember release (Yearly Electricity Data 2025): UK 217.33, EU 209.64, World 458.54 g CO₂e/kWh, replacing a stale 494 that was Ember's World figure for roughly 2020–21. Because SWD v4's published rating bands were calibrated against 494, both are rescaled by the same ratio, then to 0.111 and 0.334 g at that release — a deliberate, documented divergence from SWD's own thresholds, since keeping them unchanged against a lower world intensity would flatter every site. The Carbon Reduction Plan data block uses the identical calculation and states the audience mix, grid source and band rescaling. **The constants above are the ones that release introduced and are kept as the record; they were re-based on 9 September 2026 to a single dated Ember retrieval — UK 222.18, EU 210.13, World 459.03, thresholds 0.1115 and 0.3345 — under calculator methodology v1.7, because the original retrieval date had never been recorded and the vintage could not afterwards be established.** Methodology document to v1.4. **Carbon figures published on any earlier version are overstated for a low-carbon host and require restating.**
...
- [Ember — Yearly Electricity Data](https://ember-energy.org/data/yearly-electricity-data/) — annual electricity generation and grid emissions intensity by country, revised each year; the source for the grid figures in criterion 1 and for the world average used by Hive IT's calculators.
...
**A note on user location and grid intensity.** Where an AI provider processes your queries matters. On Ember's 2025 data, China's grid averages **528 g CO₂e/kWh** against a world average of 459, while EU member states span **35 (Sweden) to 591 (Poland)**, averaging 210 across the EU-27. So "in the EU" is not by itself a low-carbon claim: three of the largest EU data-centre markets — Germany (334), Ireland (263) and the Netherlands (254) — sit well above that average. Ask which country, not which bloc. Some providers use market-based carbon accounting (buying renewable energy certificates globally) which can understate the location-based impact for users in high-carbon grid areas, or overstate it for users on clean-grid infrastructure. When scoring, note which accounting method the provider uses, and consider the grid intensity of where your queries will actually be processed. For open-weight models, self-hosting on low-carbon infrastructure is a genuine option that can transform this score.
...
**Why it matters.** Data centres use water to cool equipment, and the power stations that supply them use more. Water stress is a **local** problem: the same litre matters far more in a drought-hit region than in a wet one. Crucially, most of that water is not used on-site: for US data centres in 2018, about three-quarters of the **operational** water footprint came from generating their electricity, not from cooling at the facility. Weighting for local water stress shifts the balance back — in the same study, more than 40% of the scarcity-weighted impact was direct, because on-site cooling clusters in stressed basins. Generative AI models generate significant heat that requires water to dissipate: for GPT-3, a widely cited estimate is roughly 0.5 litres of freshwater per 10 to 50 medium-length responses — a response assuming about 800 words of input and 150 to 300 words of output. This varies significantly by model, data-centre design and local climate. Figures are often incomplete and contested, so treat single numbers with care.
...
- Li, P., Yang, J., Islam, M. A. and Ren, S. (2025). *Making AI Less “Thirsty”: Uncovering and Addressing the Secret Water Footprint of AI Models*. Communications of the ACM, 68(7). Originally arXiv:2304.03271 (2023). — estimates per-response water consumption across AI providers.
- Siddik, M. A. B., Shehabi, A. and Marston, L. (2021). *The environmental footprint of data centers in the United States*. Environmental Research Letters, 16(6), 064017. [https://doi.org/10.1088/1748-9326/abfba1](https://doi.org/10.1088/1748-9326/abfba1) — the direct/indirect split in US data-centre water use, and an AWARE-weighted scarcity footprint alongside the volumetric one.
...
- [AWARE (Available WAter REmaining) — WULCA](https://wulca-waterlca.org/what-is-aware/) — the consensus water-scarcity weighting method used to adjust water-consumption volumes for local scarcity, recommended by the UNEP-SETAC Life Cycle Initiative, the European Commission's PEF/OEF programme and the International EPD System. Method paper: Boulay, A.-M., Bare, J., Benini, L. et al. (2018). *The WULCA consensus characterization model for water scarcity footprints: assessing impacts of water consumption based on available water remaining (AWARE)*. The International Journal of Life Cycle Assessment, 23(2), 368–378. [https://doi.org/10.1007/s11367-017-1333-8](https://doi.org/10.1007/s11367-017-1333-8)
- Siddik, M. A. B., Shehabi, A. and Marston, L. (2021). *The environmental footprint of data centers in the United States*. Environmental Research Letters, 16(6), 064017. [https://doi.org/10.1088/1748-9326/abfba1](https://doi.org/10.1088/1748-9326/abfba1) — the direct/indirect split in US data-centre water use, and an AWARE-weighted scarcity footprint alongside the volumetric one.
...
- Li, P., Yang, J., Islam, M. A. and Ren, S. (2025). *Making AI Less “Thirsty”: Uncovering and Addressing the Secret Water Footprint of AI Models*. Communications of the ACM, 68(7). Originally arXiv:2304.03271 (2023). [https://arxiv.org/abs/2304.03271](https://arxiv.org/abs/2304.03271) — estimates per-response water consumption across AI providers and distinguishes direct and indirect water scopes.
